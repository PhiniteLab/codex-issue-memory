...
class TestAsymmetricFPCost(unittest.TestCase):
    ...
    def test_fp_reward_is_more_costly_than_rejection(self):
        ...
        self.assertLess(
            DEFAULT_REWARDS["false_positive"], DEFAULT_REWARDS["candidate_rejected"]
        )
    ...
    def test_safe_override_blocks_fp_candidate(self):
        ...
        from codex_issue_memory.learning.safe_override import (
            SafeOverridePolicy,
        ...
        result = policy.choose(
            ...
            baseline_score=0.50,
            analyses={
                "baseline": FakeAnalysis(
                    final_score=0.50,
                    conservative_score=0.50,
                    effective_evidence=50.0,
                    fp_count=0,
                ),
    ...
    def test_safe_override_allows_low_fp_candidate(self):
        ...
        result = policy.choose(
            ...
            analyses={
                "baseline": FakeAnalysis(
                    final_score=0.50, conservative_score=0.50, effective_evidence=50.0
                ),
...
class TestFeedbackDrivenCalibration(unittest.TestCase):
    ...
    def test_calibration_with_no_feedback_data(self):
        ...
        from codex_issue_memory.benchmarks.calibration import (
...
class TestFamilySpecificWeights(unittest.TestCase):
    ...
    def test_ranker_family_overrides_merge_correctly(self):
        ...
        self.assertEqual(
            weights["proven_score"], HeuristicRanker.DEFAULT_WEIGHTS["proven_score"]
        )
    ...
    def test_ranker_ignores_invalid_weight_keys_in_overrides(self):
        ...
        self.assertEqual(
            weights["family_score"], HeuristicRanker.DEFAULT_WEIGHTS["family_score"]
        )
...
class TestVelocityIntegration(unittest.TestCase):
    ...
    def test_velocity_passed_to_all_posteriors(self):
        ...
        ranked_item = RankedCandidate(
            candidate=candidate, score=0.7, features={}, reasons=[]
        )
        ...
        with patch.object(
            app.store, "query_repo_feedback_velocity", side_effect=track_velocity
        ):
            bandit.store = app.store
            bandit.score_candidates(profile, [ranked_item], project_scope="global")
    ...
    def test_velocity_affects_effective_half_life(self):
        ...
        self.assertGreater(
            a2, a1, "Higher velocity should retain more evidence (higher alpha)"
        )
...
class TestFamilyWeightsScoring(unittest.TestCase):
    ...
    def test_family_overrides_change_score(self):
        ...
        result_no_family = ranker.score(
            profile, candidate, project_scope="global", error_family=""
        )
        # Now add a family override that radically changes weights
        ranker._weight_overrides = {
            "sqlite_error": {"dense_score": 0.80, "family_score": 0.01},
        }
        result_with_family = ranker.score(
            profile, candidate, project_scope="global", error_family="sqlite_error"
        )
...
class TestNegativeApplicabilityPenalty(unittest.TestCase):
    ...
    def test_penalty_3_tuple_with_fp_data(self):
        ...
        penalty, reasons, fp_count = bandit._negative_applicability_penalty(
            profile, variant
        )
    ...
    def test_fp_count_propagates_to_outcome(self):
        ...
        results = bandit.score_candidates(
            profile, [ranked_item], project_scope="global"
        )
...
class TestCLICalibrateFromFeedbackWriteProfile(unittest.TestCase):
    ...
    def test_from_feedback_write_profile_produces_file(self):
        ...
        if result.get("global"):
            ...
            self.assertTrue(
                profile_path.exists(), "Profile file should have been written"
            )

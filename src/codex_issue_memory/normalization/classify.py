...
def classify_from_text(text: str) -> tuple[str, str, list[str], list[str]]:
    ...
    has_config_container = _contains_any(
        text,
        (
            "yaml",
            "toml",
    ...
    has_config_key_signal = (
        _contains_any(text, ("missing key", "invalid key", "required key", "keyerror"))
        or (
            _contains_any(text, ("not found in", "absent from"))
            and _contains_any(
                text, ("yaml", "toml", "config", "configuration", "settings")
            )
        )
        or (
    ...
    if _contains_any(
        text, ("permission denied", "operation not permitted", "access is denied")
    ):
    ...
    if _contains_any(
        text, ("wrong unicode codec", "wrong codec", "wrong encoding", "codec mismatch")
    ) and _contains_any(
        text,
    ...
    if _contains_any(
        text, ("keyerror", "column", "columns", "dataframe")
    ) and _contains_any(
        text,
    ...
    if _contains_any(text, ("publickey", "ssh", "git remote")) and _contains_any(
        text, ("permission denied", "auth", "authentication")
    ):
        family = "auth_error"
    ...
    if (
        _contains_any(text, ("assertionerror", "assert ", "expected", "but got"))
        and family == "generic_runtime_error"
    ):
    ...
    if _contains_any(
        text, ("lora", "adapter", "checkpoint", "base model")
    ) and _contains_any(
        text,
        (
            "rank",
    ...
    if _contains_any(
        text, ("out of memory", "cuda out of memory", "oom", "killed process")
    ):
    ...
    if _contains_any(
        text, ("env var", "environment variable", "not set", "keyerror")
    ) and _contains_any(
        text,
        (
            "dotenv",

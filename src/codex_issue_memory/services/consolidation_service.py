...
class ConsolidationService:
    ...
    def plan(
        ...
        *,
        ...
        existing_pattern = self.store.find_pattern_by_signature(
            project_scope, proposed_pattern_key
        )
        if existing_pattern is not None:
            pattern_id = int(existing_pattern["id"])
            pattern_signature = str(existing_pattern["signature"])
            exact_variant = self.store.find_variant_by_key(
                pattern_id=pattern_id, variant_key=proposed_variant_key
            )
            ...
            return ConsolidationPlan(
                matched_pattern_id=pattern_id,
                matched_variant_id=best_variant_id
                if best_score >= self.EXACT_PATTERN_VARIANT_THRESHOLD
                else None,
                pattern_signature=pattern_signature,
                proposed_pattern_key=proposed_pattern_key,
                proposed_variant_key=proposed_variant_key,
                ...
                variant_strategy=(
                    ...
                    and best_score >= self.EXACT_PATTERN_VARIANT_THRESHOLD
                    else "new_variant_in_exact_pattern"
                ),
                consolidation_score=round(best_score, 4),
                reasons=["exact-pattern-key", *reasons],
                requires_review=best_variant_id is not None
                and (self.EXACT_PATTERN_VARIANT_THRESHOLD - 0.08)
                <= best_score
                < self.EXACT_PATTERN_VARIANT_THRESHOLD,
            )
        ...
        for item in ranked:
            ...
            variant_id = (
                int(candidate_variant["id"])
                if isinstance(candidate_variant, dict)
                and candidate_variant.get("id") is not None
                else None
            )
            variant_score = 0.0
            reasons = list(item.reasons)
            if variant_id is not None:
                variant_score, variant_reasons = self._variant_similarity(
                    profile,
                    candidate_variant,
                    proposed_variant_key=proposed_variant_key,
                )
            ...
            if strong_pattern_attach:
                return ConsolidationPlan(
                    ...
                    variant_strategy=(
                        "retrieval_variant_merge"
                        if attach_variant is not None
                        else "retrieval_new_variant"
                    ),
    ...
    def _best_variant_for_pattern(
        ...
        *,
        ...
        for variant in self.store.get_variants_for_pattern(pattern_id, limit=50):
            score, reasons = self._variant_similarity(
                profile, variant, proposed_variant_key=proposed_variant_key
            )
            if score > best_score:
    ...
    def _variant_similarity(
        ...
        *,
        ...
        variant_tokens = tokenize(
            " ".join(
                [
                    ...
                    " ".join(
                        str(tag)
                        for tag in variant.get("tags_json", [])
                        if isinstance(variant.get("tags_json", []), list)
                    ),
                ]
            ),
            max_tokens=96,
        ...
        if profile.tokens and variant_tokens:
            overlap = len(set(profile.tokens) & set(variant_tokens)) / max(
                1, len(set(profile.tokens) | set(variant_tokens))
            )
        ...
        entity_slots = (
            variant.get("entity_slots_json")
            if isinstance(variant.get("entity_slots_json"), dict)
            else {}
        )
        ...
        if (
            strategy_key
            and profile.strategy_hints
            and strategy_key in profile.strategy_hints
        ):
            score += 0.08
        ...
        success_ratio = (success_count + 1.0) / max(
            success_count + reject_count + 2.0, 1.0
        )
        reject_ratio = reject_count / max(success_count + reject_count + 1.0, 1.0)
        feedback_weight = max(
            0.0,
            0.40 * confidence
            + 0.35 * memory_strength
            + 0.25 * success_ratio
            - 0.20 * reject_ratio,
        )
